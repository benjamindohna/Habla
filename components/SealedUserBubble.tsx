 ...
 * we have the corrected target-language text but not the full
 * in-memory CorrectionResult (interpretation, confidence, notes), so
 * the rich correction view from UserBubble can't render. Just shows
 * the corrected text.
 *
 ...
 */
export default function SealedUserBubble({ textTarget }: { textTarget: string }) {
  return (
    <div className="flex justify-end">
      <div className="max-w-[80%] rounded-2xl bg-neutral-900 px-4 py-3 text-base leading-relaxed text-white">
        <span className="whitespace-pre-wrap">{textTarget}</span>
      </div>
    </div>
  );
}
